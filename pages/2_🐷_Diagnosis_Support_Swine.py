...
def load_diseases():
    ...
    df["key_symptoms_list"] = df["key_symptoms"].apply(_split_list)
    df["risk_factors_list"] = df["risk_factors"].apply(_split_list)
    return df


def _split_list(x):
    """Splits a ';'-separated cell into a clean list, tolerating blank or
    missing cells (important once the CSV is edited by hand through the
    web UI, where a row might be left partially filled in)."""
    if pd.isna(x):
        return []
    return [s.strip() for s in str(x).split(";") if s.strip()]


def load_diseases_raw():
    """Unprocessed read of diseases.csv — used only by the admin editor tab,
    so the derived *_list helper columns never get written back to disk."""
...
st.warning(
    "📖 หน้านี้เป็นเวอร์ชันสำหรับใช้ทบทวนบทเรียน ข้อมูลยังไม่ได้ผ่านการตรวจสอบอย่างละเอียด "
    "โปรดตรวจสอบกับแหล่งอ้างอิงทางวิชาการก่อนนำไปใช้ในการตัดสินใจจริง"
)

tab_analyze, tab_history, tab_admin = st.tabs(["🔍 วิเคราะห์เคสใหม่", "📋 ประวัติเคสย้อนหลัง", "⚙️ จัดการฐานข้อมูลโรค"])
...
# ============================================================
# TAB 3: จัดการฐานข้อมูลโรค (แก้ไข/เพิ่ม/ลบ ผ่านหน้าเว็บโดยตรง)
# ============================================================
with tab_admin:
    st.subheader("⚙️ จัดการฐานข้อมูลโรค (diseases.csv)")
    st.caption(
        "แก้ไขค่าในตารางได้โดยตรง เพิ่มแถวใหม่ด้วยปุ่ม '+' ท้ายตาราง หรือลบแถวโดยเลือกแล้วกดปุ่มลบ "
        "จากนั้นกด 'บันทึกการเปลี่ยนแปลง' ระบบจะเขียนทับไฟล์ diseases.csv ให้ทันที ไม่ต้องเปิดโค้ดหรือไฟล์ CSV เอง"
    )
    st.info(
        "💡 คอลัมน์ **อาการหลัก** และ **ปัจจัยเสี่ยง** ใช้เครื่องหมายอัฒภาค ( ; ) คั่นระหว่างรายการ "
        "เช่น `ไข้; ซึม; ผื่นแดงที่ผิวหนัง`"
    )

    raw_diseases_df = load_diseases_raw()

    backup_csv = raw_diseases_df.to_csv(index=False).encode("utf-8-sig")
    st.download_button(
        "⬇️ ดาวน์โหลดสำรองไฟล์ปัจจุบันก่อนแก้ไข", backup_csv,
        f"diseases_backup_{datetime.now().strftime('%Y%m%d_%H%M')}.csv", "text/csv",
    )

    edited_diseases_df = st.data_editor(
        raw_diseases_df,
        num_rows="dynamic",
        use_container_width=True,
        key="diseases_editor",
        column_config={
            "disease": st.column_config.TextColumn("โรค", required=True),
            "description": st.column_config.TextColumn("คำอธิบาย"),
            "reference": st.column_config.TextColumn("อ้างอิง"),
            "key_symptoms": st.column_config.TextColumn("อาการหลัก (คั่นด้วย ;)"),
            "risk_factors": st.column_config.TextColumn("ปัจจัยเสี่ยง (คั่นด้วย ;)"),
            "morbidity_min": st.column_config.NumberColumn("Morbidity min (%)", min_value=0, max_value=100),
            "morbidity_max": st.column_config.NumberColumn("Morbidity max (%)", min_value=0, max_value=100),
            "mortality_min": st.column_config.NumberColumn("Mortality min (%)", min_value=0, max_value=100),
            "mortality_max": st.column_config.NumberColumn("Mortality max (%)", min_value=0, max_value=100),
            "age_group": st.column_config.TextColumn("ช่วงอายุ"),
            "sample_type": st.column_config.TextColumn("ตัวอย่างที่ควรเก็บ"),
            "gold_standard_test": st.column_config.TextColumn("Gold standard test"),
            "other_tests": st.column_config.TextColumn("การตรวจเสริม/อื่นๆ"),
            "notify_authority": st.column_config.SelectboxColumn(
                "ต้องแจ้งกรมปศุสัตว์?", options=["Yes", "No"]
            ),
        },
    )
    ...
    with col_save:
        if st.button("💾 บันทึกการเปลี่ยนแปลง", type="primary", use_container_width=True):
            cleaned_df = edited_diseases_df.copy()
            cleaned_df["disease"] = cleaned_df["disease"].astype(str).str.strip()
            cleaned_df = cleaned_df[cleaned_df["disease"] != ""]  # drop fully-blank new rows

            if cleaned_df["disease"].duplicated().any():
                dupes = cleaned_df.loc[cleaned_df["disease"].duplicated(keep=False), "disease"].unique()
                st.error(
                    f"มีชื่อโรคซ้ำกัน: {', '.join(dupes)} — กรุณาแก้ให้แต่ละแถวมีชื่อโรคไม่ซ้ำกันก่อนบันทึก"
                )
            else:
                cleaned_df.to_csv(CSV_PATH, index=False, quoting=csv_module.QUOTE_ALL)
                load_diseases.clear()  # invalidate cache so the analyze tab sees the update immediately
                st.session_state.pop("diseases_editor", None)
                st.success(f"บันทึกแล้ว — ตอนนี้มีทั้งหมด {len(cleaned_df)} โรคในฐานข้อมูล")
                st.rerun()
    with col_cancel:
        if st.button("↩️ ยกเลิกการแก้ไข (โหลดค่าล่าสุดจากไฟล์ใหม่)", use_container_width=True):
            st.session_state.pop("diseases_editor", None)
            st.rerun()

st.divider()
st.caption("จัดการฐานข้อมูลโรคทั้งหมด (เพิ่ม/แก้ไข/ลบ) ได้ที่แท็บ '⚙️ จัดการฐานข้อมูลโรค' ด้านบน ไม่ต้องแก้ไฟล์ diseases.csv เอง")
